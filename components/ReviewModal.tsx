...
import { X, AlertTriangle, FileText, Download, Trash2, Loader2, Eye, EyeOff, Info } from 'lucide-react';
...
const ReviewModal: React.FC<ReviewModalProps> = ({ 
  ...
  // Track the IDs that were present when the modal opened. 
  const initialIdsRef = useRef<Set<string>>(new Set());

  useEffect(() => {
    if (isOpen) {
      // Snapshot the items when modal opens. 
  ...
  const isFlagged = (item: ReviewItem) => {
    return (item.segment.needsReview === true) || (item.segment.confidence < minConfidence);
  };
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-7xl h-[85vh] flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
        ...
        <div className="px-6 py-4 border-b border-slate-200 bg-slate-50 flex justify-between items-center flex-shrink-0">
          <div>
            ...
            <p className="text-sm text-slate-500">
              Review extracted documents, edit filenames, or remove invalid deliveries.
            </p>
          </div>
          ...
          <div className="flex items-center space-x-4">
             ...
             <div className="bg-white border border-slate-300 rounded-lg p-1 flex">
                <button
                    ...
                    className={`px-3 py-1.5 text-xs font-bold rounded-md transition-all flex items-center ${filterMode === 'all' ? 'bg-slate-800 text-white shadow' : 'text-slate-500 hover:bg-slate-50'}`}
                >
                    <Eye size={14} className="mr-1.5" /> Show All
                </button>
                <button
                    onClick={() => setFilterMode('flagged')}
                    className={`px-3 py-1.5 text-xs font-bold rounded-md transition-all flex items-center ml-1 ${filterMode === 'flagged' ? 'bg-amber-100 text-amber-700 shadow ring-1 ring-amber-200' : 'text-slate-500 hover:bg-slate-50'}`}
                >
             ...
             <div className="h-full flex flex-col items-center justify-center text-slate-400">
                {filterMode === 'flagged' ? (
                    <>
                        ...
                        <p>No issues flagged by AI.</p>
                        <button onClick={() => setFilterMode('all')} className="mt-2 text-brand-600 text-sm hover:underline">Show all documents</button>
                    </>
                ) : (
                    <>
          ...
          <table className="w-full text-left text-sm border-collapse">
            <thead className="bg-slate-100 text-slate-600 font-medium sticky top-0 z-10 shadow-sm">
              <tr>
                ...
                <th className="px-6 py-3 border-b border-slate-200 w-1/3">Filename (Editable)</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredItems.map((item) => {
                 ...
                 return (
                    <tr key={item.id} className={`transition-colors group ${flagged ? 'bg-amber-50/60 hover:bg-amber-50' : 'hover:bg-slate-50'}`}>
                    <td className="px-6 py-3 align-top">
                        <span className={`inline-flex items-center px-2 py-1 rounded text-xs font-bold ${getConfidenceColor(item.segment.confidence)}`}>
                        {item.segment.confidence < minConfidence && <AlertTriangle size={12} className="mr-1" />}
                        {(item.segment.confidence * 100).toFixed(0)}%
                        </span>
                        
                        {/* Review Reason Badge */}
                        {(item.segment.needsReview || item.segment.confidence < minConfidence) && (
                            <div className="mt-2 text-[10px] leading-tight text-amber-800 bg-amber-100 p-2 rounded border border-amber-200 max-w-[120px]">
                                <strong className="block mb-1 flex items-center"><Info size={10} className="mr-1"/> AI Flag:</strong>
                                {item.segment.reviewReason || (item.segment.confidence < minConfidence ? "Low confidence score" : "Review requested")}
                            </div>
                        )}
                    </td>
                    ...
                    <td className="px-6 py-3 align-top pt-3">
                        <div className="relative">
                            <input
                                ...
                                disabled={isSaving}
                                className={`w-full px-3 py-1.5 border rounded focus:ring-2 focus:ring-brand-500 focus:border-brand-500 transition-all font-medium text-slate-800 disabled:opacity-60 ${flagged ? 'border-amber-300 bg-white' : 'border-slate-300'}`}
                            />
                    ...
                    <td className="px-6 py-3 align-top pt-3">
                        <div className="text-xs space-y-1">
                            <div className="flex">
                                <span className="text-slate-400 w-16">ID:</span>
                                <span className={`font-medium ${flagged ? 'text-amber-900 bg-amber-100 px-1 rounded' : 'text-slate-700'}`}>{item.segment.deliveryId}</span>
                            </div>
                            <div className="flex">
                            ...
                            {item.segment.customerId && (
                                <div className="flex">
                                    <span className="text-slate-400 w-16">Cust ID:</span>
                                    <span className="text-slate-700 font-medium">{item.segment.customerId}</span>
                                </div>
                            )}
                            <div className="flex">
        ...
        <div className="px-6 py-4 bg-slate-50 border-t border-slate-200 flex justify-between items-center flex-shrink-0">
          <div className="text-sm text-slate-500">
             ...
             {items.filter(i => isFlagged(i)).length > 0 && (
                 <span className="ml-2 text-amber-600">({items.filter(i => isFlagged(i)).length} need review)</span>
             )}
          </div>
